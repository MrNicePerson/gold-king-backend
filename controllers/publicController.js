...
export const getPublicPictures = async (req, res) => {
  try {
    const pictures = await Picture.find({
      ...
      .populate({
        path: "uploadedBy",
        select: "shopName phoneNumber whatsappNumber whatsappLink diff_24k diff_2385k diff_silver",
      })
      .sort({ createdAt: -1 })
      .lean();
    ...
    const formattedPictures = pictures.map((picture) => ({
      ...
      shop: picture.uploadedBy
        ? {
          ...
          diff_24k: picture.uploadedBy.diff_24k ?? 0,
          diff_2385k: picture.uploadedBy.diff_2385k ?? 0,
          diff_silver: picture.uploadedBy.diff_silver ?? 0,
        }
        : null,
    }));
